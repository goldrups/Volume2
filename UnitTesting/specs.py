...
def smallest_factor(n):
    ...
    if n == 1: return 1
    for i in range(2, int(n)): #used to be n**0.5
        if n % i == 0: return i
    return n
...
def count_sets(cards):
    ...
    if len("".join(cards)) != 48:
        raise ValueError("one or more cards does not have exactly 4 digits")
    wrong_entry_list = [s for s in "".join(cards) if int(s) < 0 or int(s) > 2]
    if len(wrong_entry_list) > 0:
        raise ValueError("one or more cards has a character other than 0,1, or 2")

    combos = list(combinations(cards, 3)) #creates a list of combinations of length 3
    num_sets = 0
    for combo in combos: #iterates on the differnt combinations
        a_card, b_card, c_card = combo
        if is_set(a_card, b_card, c_card) == True:
            num_sets += 1
    return num_sets
...
def is_set(a, b, c):
    ...
    """
    dig_sums = [(int(a[i]) + int(b[i]) + int(c[i])) for i in range(len(a))] #returns list of sums of each digit
    set_check = [dig_sums[i] % 3 == 0 for i in range(len(a))] #checks to see if each dig sum is divisible by 3
    return sum(set_check) == 4 #returns True if each element is True
